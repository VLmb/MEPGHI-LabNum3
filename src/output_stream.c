...
void WriteDequeToFile(const char* output_file, Deque* deq){
    ...
    Element* cur_el = deq->begin;
    while (cur_el != NULL){
        fprintf(file, "%d", cur_el->value);
        if (cur_el->next != NULL){
            fprintf(file, ",");      
        }
        cur_el = cur_el->next;
...
void PrintData(const char* input_file, const char* output_file){
    if (input_file == NULL){
        scanf("File or path to file : %s", input_file);
    }
    int count;
    ApartmentBuilding* buildings = ReadApartmentBuildingsFromFile(input_file, &count);
    WriteToFileTable(buildings, count, output_file);
}